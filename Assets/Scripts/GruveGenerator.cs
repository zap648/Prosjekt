        ...
        while (iterations < 100)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (Random.Range(0, 100) < 5)
                {
                    ...
                    {
                        queue.Add(new Cell(queue[iterations].coordinates[0], queue[iterations].coordinates[0] + 1));
                        queue.Last().neighbour[2] = true;
                    }
                    else if (i == 1) // If the neighbour is rightside (1 - right)
                    {
                        queue.Add(new Cell(queue[iterations].coordinates[0] + 1, queue[iterations].coordinates[0]));
                    ...
                    newRoom.name += $" {currentCell.coordinates[0]}-{currentCell.coordinates[1]}";
                }
            }
            iterations++;
        }
